...
class PolicyInference(object):
    ...
    def gradientAscentHyperParamOpt(self, histories, theta_0=None, do_print=False, use_precomputed_phi=False,
                                    dtype=np.float64, monte_carlo_size=None, reference_policy_vec=None,
                                    precomputed_observed_action_indeces=None):
        """
        @brief Performs Policy inference using Gradient Ascent from Section 7.2.1 of
               Sugiyama, 2015.
        ...
        if precomputed_observed_action_indeces is not None:
            ...
            # Precthetaompute observed actions for all episodes.
            observed_action_indeces = np.empty([num_episodes, num_steps], dtype=np.int8)
            for episode in xrange(num_episodes):
                for t_step in xrange(1, num_steps):
                    this_state = self.histories[episode, t_step-1]
        ...
        def forward_mode():
                ...
                for trial in xrange(num_inferences):
                    global is_last_trial, trial_tic,trial
                    is_last_trial = True if (num_inferences - trial)==1 else False
                    trial_tic = time.clock()
                    inverse_temp = inverse_temp_start
                    iter_count = 0
                    ...
                    global theta
                    # Loop until convergence
                    Z=np.zeros([2*theta_size,self.mdp.num_kern])
                    while delta_theta_norm > thresh:
                        if do_print:
                            iter_tic = time.clock()
                        iter_count += 1
                        ...
                        while len(traj_queue)>0:
                            ...
                            if use_precomputed_phi:
                                ...
                                global exp_Q,sum_weighted_exp_Q,reciprocal_sum_exp_Q
                                exp_Q = np.exp(np.einsum('ijk,hk->ij', phis, theta))
                                reciprocal_sum_exp_Q = np.reciprocal(np.einsum('ij->i', exp_Q))
                                sum_weighted_exp_Q = np.einsum('ij,ijk->ki', exp_Q, phis)
                                # For this calc only because einsum uses a pedantic alphabetic convention:
                        ...
                        Z=np.dot(d_phi_ds(),Z)+d_phi_dlamda()
                ...
                for i in range(theta_size):
                    ...
                    for state in self.mdp.state_vec:
                        ...
                        for act2_idx, act2 in enumerate(acts_list):
                                n1+=np.inner(phis[state,act2_idx,:]*exp_Q[state,act2_idx],phis[state,act2_idx,:]*exp_Q[state,act2_idx])
                                n2+=np.inner(phis[state,act2_idx,:]*exp_Q[state,act2_idx],phis[state,act2_idx,:])
                        i1=0
                        for act_id, act in enumerate(acts_list):
                            i1+=n1/(exp_Q[state,act_id]**2)-n2/(exp_Q[state,act_id])
                        i2+=i1
                    grad[0,i]=i2
                return np.concatenate((np.zeros([1,theta_size]),grad),axis=1)
            ...
            for state in self.mdp.state_vec:
                    ...
                    for act_idx, act in enumerate(acts_list):
                        for kern_idx in range(theta_size):
                            ...
                            db=0
                            for act2_idx, act2 in enumerate(acts_list):
                                da+=phis[state,act2_idx, :]*exp_Q[state,act2_idx]
                                db+=np.outer(phis[state,act2_idx, :],phis[state,act2_idx, :])*exp_Q[state,act2_idx]

                    ca+=-db/np.sum(exp_Q[state,:])+np.outer(da,da)/(np.sum(exp_Q[state,:])**2)
            return ca
        ...
        def del_V_del_sig():
                ...
                for state in self.mdp.state_vec:
                    for act_idx, act in enumerate(acts_list):
                        for kern_idx in range(theta_size):
                            for kern_idx2 in range(self.mdp.num_kern):
                                da=0
                                db=0
                                for act2_idx, act2 in enumerate(acts_list):
                ...
                return np.sum(np.sum(dpds-ba-dd+ca,axis=0),axis=0)

                #second=(dpds*exp_Q) exp_Q[state,:].T/np.sum(np.einsum('ijk,hk->ij', phis, theta),axis=1)
                #for state in self.mdp.state_vec:
                 #   for act_idx, act in enumerate(acts_list):
        ...
        def del_phi_del_sig():
                del_phis = np.zeros([self.mdp.num_states,self.mdp.num_actions,theta_size, self.mdp.num_kern], dtype=dtype)
                ns=self.mdp.num_states
                na=self.mdp.num_actions
                nk=self.mdp.num_kern
                for state in self.mdp.state_vec:
                    for act_idx, act in enumerate(acts_list):
                        for kern_idx in range(self.mdp.num_kern):
                            for kern_idx2 in range(theta_size):
                                del_phis[state,act_idx,kern_idx2, kern_idx] = self.mdp.delPhi_delSig[kern_idx, state,act_idx]
                return del_phis
